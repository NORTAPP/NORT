...
export default function LiquidEther({
  ...
  void main(){
    float x0 = texture2D(velocity, uv-vec2(px.x,0.)).x; float x1 = texture2D(velocity, uv+vec2(px.x,0.)).x;
    float y0 = texture2D(velocity, uv-vec2(0.,px.y)).y; float y1 = texture2D(velocity, uv+vec2(0.,px.y)).y;
  ...
  void main(){
    float p0=texture2D(pressure,uv+vec2(px.x*2.,0.)).r; float p1=texture2D(pressure,uv-vec2(px.x*2.,0.)).r;
    float p2=texture2D(pressure,uv+vec2(0.,px.y*2.)).r; float p3=texture2D(pressure,uv-vec2(0.,px.y*2.)).r;
  ...
  void main(){
    float p0=texture2D(pressure,uv+vec2(px.x,0.)).r; float p1=texture2D(pressure,uv-vec2(px.x,0.)).r;
    float p2=texture2D(pressure,uv+vec2(0.,px.y)).r; float p3=texture2D(pressure,uv-vec2(0.,px.y)).r;
  ...
  void main(){
    ...
    vec2 n0=texture2D(velocity_new,uv+vec2(px.x*2.,0.)).xy; vec2 n1=texture2D(velocity_new,uv-vec2(px.x*2.,0.)).xy;
    vec2 n2=texture2D(velocity_new,uv+vec2(0.,px.y*2.)).xy; vec2 n3=texture2D(velocity_new,uv-vec2(0.,px.y*2.)).xy;
    vec2 newv = 4.*old + v*dt*(n0+n1+n2+n3);
    newv /= 4.*(1.+v*dt);
    ...
    class Simulation {
      ...
      createShaderPass() {
        this.advection = new Advection({ cellScale:this.cellScale, fboSize:this.fboSize,
          dt:this.options.dt, src:this.fbos.vel_0, dst:this.fbos.vel_1 });
        this.externalForce = new ExternalForce({ cellScale:this.cellScale,
          cursor_size:this.options.cursor_size, dst:this.fbos.vel_1 });
        this.viscous = new Viscous({ cellScale:this.cellScale, boundarySpace:this.boundarySpace,
        ...
        this.divergence = new Divergence({ cellScale:this.cellScale, boundarySpace:this.boundarySpace,
          src:this.fbos.vel_viscous0, dst:this.fbos.div, dt:this.options.dt });
        this.poisson = new Poisson({ cellScale:this.cellScale, boundarySpace:this.boundarySpace,
          src:this.fbos.div, dst:this.fbos.pressure_1, dst_:this.fbos.pressure_0 });
        this.pressure = new Pressure({ cellScale:this.cellScale, boundarySpace:this.boundarySpace,
    ...
    if (sim) {
      ...
      Object.assign(sim.options, { mouse_force:mouseForce, cursor_size:cursorSize,
